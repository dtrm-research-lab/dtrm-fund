...
STATEMENT = ROOT / "research/contracts/DTRM_PHASE4_SCHEDULED_REVISION_ADDENDUM_STATEMENT_V1.json"
...
def test_independent_backend_git_object_evidence_gates_registered_revision() -> None:
    """Cross-check the contract against evidence read independently from backend Git objects."""
    ...
    assert observed["scheduled_job_arm_expression"] == "github.event_name == 'schedule' && vars.PHASE4_PROSPECTIVE_CAPTURE_ARMED == 'true'"
    ...
    assert observed["provider_credential_present_in_checkout_setup_install_or_preflight"] is False
...
def test_exact_merged_backend_revision_and_workflow_are_bound() -> None:
    ...
    assert isinstance(backend, dict) and isinstance(workflow, dict) and isinstance(provider, dict)
...
@pytest.mark.parametrize(("field", "value"), [
    ("periodic_capture_activation_permitted", True), ("prospective_start_bound", True),
    ("credential_or_writer_authority_claimed", True), ("activation_statement_provisioned", True),
    ...
    ("scheduled_rerun_counting_eligible", True), ("manual_workflow_dispatch_counting_eligible", True),
    ("scheduled_side_effects_permitted_after_final_slot", True), ("scheduled_target_slot_min", -1),
    ("scheduled_target_slot_max", 56),
])
def test_runtime_promotions_fail_closed(field: str, value: object) -> None:
    payload = expected_statement(); semantics = payload["runtime_semantics"]
    assert isinstance(semantics, dict); semantics[field] = value
    with pytest.raises(ScheduledRevisionAddendumError): verify_statement_bytes(_bytes(payload))


def test_backend_or_workflow_drift_fails_closed() -> None:
    payload = expected_statement(); backend = payload["backend_revision"]
    assert isinstance(backend, dict); backend["merge_commit"] = VALIDATED_PR_HEAD
    with pytest.raises(ScheduledRevisionAddendumError): verify_statement_bytes(_bytes(payload))
    payload = expected_statement(); workflow = payload["workflow"]
    assert isinstance(workflow, dict); workflow["blob_sha"] = "0" * 40
    with pytest.raises(ScheduledRevisionAddendumError): verify_statement_bytes(_bytes(payload))


def test_duplicate_keys_and_secret_shaped_material_fail_closed() -> None:
    raw = STATEMENT.read_text()
    duplicated = raw.replace('"schema_version": "dtrm.phase4.scheduled_revision_addendum.v1",', '"schema_version": "dtrm.phase4.scheduled_revision_addendum.v1",\n  "schema_version": "dtrm.phase4.scheduled_revision_addendum.v1",', 1)
    with pytest.raises(ScheduledRevisionAddendumError, match="duplicate JSON keys"): verify_statement_bytes(duplicated.encode())
    payload = expected_statement(); workflow = payload["workflow"]
    assert isinstance(workflow, dict); workflow["arm_variable"] = "mongodb+srv://redacted.invalid"
    with pytest.raises(ScheduledRevisionAddendumError, match="forbidden secret material"): verify_statement_bytes(_bytes(payload))
