...
import run_phase3_mm1_v6_realized_evaluation as realized
    ...
    return pd.DataFrame(
        {
            ...
            "date_dt": pd.to_datetime(
                ["2026-06-26", "2026-06-27", "2026-07-04", "2026-07-06"],
                utc=True,
            ),
        }
    )
...
def test_selection_positions_maps_frozen_identities_exactly():
    signals = _signals()
    positions = realized.selection_positions(
        signals,
        [_identity(signals, 2), _identity(signals, 0)],
        k=2,
    )
...
def test_price_series_by_ticker_is_ordered_and_date_only():
    prices = pd.DataFrame(
        {
            ...
            "date": ["2026-06-27", "2026-06-26", "2026-06-27", "2026-06-26"],
    ...
    assert result["AAA"][0][0].isoformat() == "2026-06-26"


def test_build_v6_raw_targets_uses_frozen_beta_and_forward_window():
    signals = pd.DataFrame(
        {
            "news_id": ["n1"],
            "ticker": ["AAA"],
            "date_dt": pd.to_datetime(["2026-07-06"], utc=True),
        }
    )
    ...
    prices = pd.DataFrame(
        {
            "ticker": ["AAA", "AAA", "SPY", "SPY"],
            "date": ["2026-07-06", "2026-09-04", "2026-07-06", "2026-09-04"],
            "close": [100.0, 120.0, 200.0, 210.0],
        }
    )
    raw = realized.build_v6_raw_targets(signals, beta, prices)
    assert raw[0] == pytest.approx(0.10, abs=1e-12)


def test_build_v6_raw_targets_refuses_incomplete_target():
    signals = pd.DataFrame(
        {
            "news_id": ["n1"],
            "ticker": ["AAA"],
    ...
    with pytest.raises(RuntimeError, match="INFEASIBLE"):
        realized.build_v6_raw_targets(signals, beta, prices)
...
def test_runner_source_never_calls_optimizer():
    source = (EXPERIMENTS / "run_phase3_mm1_v6_realized_evaluation.py").read_text()
...
def test_runner_binds_to_v6_provenance_and_maturity_date():
    assert realized.PRICE_PROVENANCE_PATH.name == "DTRM_PHASE3_MM1_V6_REALIZED_PRICE_PROVENANCE.json"
    assert realized.EXPECTED_LATEST_TARGET_DATE == "2026-09-04"


def test_runner_outputs_are_v6_specific():
    assert realized.TARGET_OUTPUT.name == "phase3_mm1_v6_target_model.pkl"
    assert realized.RESULT_OUTPUT.name == "DTRM_PHASE3_MM1_V6_REALIZED_RESULT.json"
