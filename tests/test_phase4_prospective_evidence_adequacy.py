...
def _attempt(binding: ActivationBinding, slot: int, **overrides: object) -> SlotAttempt:
    ...
    return SlotAttempt(**values)  # type: ignore[arg-type]


def _attempts(binding: ActivationBinding, count: int = TARGET_SLOTS) -> tuple[SlotAttempt, ...]:
    return tuple(_attempt(binding, slot) for slot in range(count))


def _final_clock(binding: ActivationBinding):
...
def test_all_56_slots_pass_after_finalization() -> None:
    ...
    assert report["counts"]["ACCEPTED"] == 56  # type: ignore[index]
    assert report["counts"]["MISSING"] == 0  # type: ignore[index]
...
def test_exactly_48_slots_pass_without_backfill() -> None:
    ...
    assert report["final_status"] == "PASS_PROSPECTIVE_EVIDENCE_ADEQUACY_V1"
    assert report["counts"]["ACCEPTED"] == 48  # type: ignore[index]
    assert report["counts"]["MISSING"] == 8  # type: ignore[index]
...
def test_47_slots_fail_after_finalization() -> None:
    ...
    assert report["counts"]["ACCEPTED"] == 47  # type: ignore[index]
...
def test_manual_run_never_counts_toward_threshold() -> None:
    ...
    assert report["counts"]["ACCEPTED"] == 55  # type: ignore[index]
    assert report["counts"]["MISSING"] == 1  # type: ignore[index]
...
def test_late_attempt_is_not_accepted() -> None:
    ...
    report = audit_evidence(binding, tuple(attempts), _final_clock(binding))
    assert report["counts"]["LATE"] == 1  # type: ignore[index]
    assert report["counts"]["ACCEPTED"] == 55  # type: ignore[index]
...
def test_failed_attempt_is_not_accepted() -> None:
    ...
    report = audit_evidence(binding, tuple(attempts), _final_clock(binding))
    assert report["counts"]["FAILED"] == 1  # type: ignore[index]
...
def test_duplicate_slot_is_not_counted_twice() -> None:
    ...
    assert report["counts"]["DUPLICATE"] == 1  # type: ignore[index]
    assert report["counts"]["ACCEPTED"] == 55  # type: ignore[index]
...
def test_wrong_lineage_and_request_fingerprint_fail_closed() -> None:
    ...
    report = audit_evidence(binding, tuple(attempts), _final_clock(binding))
    assert report["counts"]["CONTRACT_MISMATCH"] == 2  # type: ignore[index]
...
def test_duplicate_run_id_across_slots_fails_closed() -> None:
    ...
    report = audit_evidence(binding, tuple(attempts), _final_clock(binding))
    assert report["counts"]["CONTRACT_MISMATCH"] == 2  # type: ignore[index]
    assert report["counts"]["ACCEPTED"] == 54  # type: ignore[index]
