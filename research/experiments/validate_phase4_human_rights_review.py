"""Validate the Phase-IV human rights review protocol without external I/O."""
...
def main(argv: list[str] | None = None) -> int:
    ...
    try:
        ...
        if statement_bytes != canonical_statement_bytes(normalized):
            raise HumanRightsReviewError("statement is not canonically serialized")
        review = review_human_rights(payload)
        with args.output.open("x", encoding="utf-8") as output:
            output.write(serialize_review(review))
    except HumanRightsReviewError:
        error = "INVALID_HUMAN_RIGHTS_REVIEW_STATEMENT"
