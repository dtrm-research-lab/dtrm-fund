"""Build the frozen realized-price snapshot for Phase-3 MM1 V5.

This is the first Stage-2 outcome-access step. It is permitted only after the
tracked V5 ex-ante decision manifest is committed. The builder snapshots prices
only; it does not construct target_model, evaluate either action, or rerun MM1.
"""
...
PRICE_OUTPUT = LOCAL_DATA / "phase3_mm1_v5_price_snapshot.pkl"
SUMMARY_OUTPUT = LOCAL_DATA / "phase3_mm1_v5_price_snapshot_summary.json"

PRICE_START = pd.Timestamp("2026-06-15")
PRICE_END = pd.Timestamp("2026-08-26")
REQUIRED_LAST_MARKET_DATE = pd.Timestamp("2026-08-24")
...
def require_manifest_committed() -> dict:
    """Require the local manifest bytes to already exist in the current Git HEAD."""

    if not MANIFEST_PATH.exists():
        raise FileNotFoundError(f"Missing V5 decision manifest: {MANIFEST_PATH}")
    ...
    try:
        ...
        raise RuntimeError(
            "V5 decision manifest must be committed before realized price access"
        ) from exc

    local = MANIFEST_PATH.read_bytes()
    if committed != local:
        raise RuntimeError(
            "Local V5 decision manifest differs from the committed Git version"
        )

    manifest = json.loads(local)
    if manifest.get("artifact") != "DTRM_PHASE3_MM1_V5_DECISION_MANIFEST":
        raise RuntimeError("Unexpected V5 decision manifest artifact identity")
    if manifest.get("status") != "decision_frozen_pending_git_commit":
        raise RuntimeError("Unexpected V5 decision manifest status")

    firewall = manifest.get("information_firewall", {})
    if firewall.get("V5_realized_outcomes_accessed") is not False:
        raise RuntimeError("V5 manifest does not certify a clean ex-ante firewall")
    if firewall.get("manifest_commit_required_before_realized_evaluation") is not True:
        raise RuntimeError("V5 manifest does not require commit before evaluation")
...
def load_frozen_signals(manifest: dict) -> pd.DataFrame:
    ...
    if expected.get("path") != expected_path:
        raise RuntimeError("V5 frozen-signal path differs from decision manifest")
    if not SIGNALS_PATH.exists():
        raise FileNotFoundError(f"Missing frozen V5 signals: {SIGNALS_PATH}")

    actual_sha = sha256_file(SIGNALS_PATH)
    if actual_sha != expected.get("sha256"):
        raise RuntimeError("V5 frozen-signal SHA-256 differs from decision manifest")

    signals = pd.read_pickle(SIGNALS_PATH)
    missing = set(REQUIRED_SIGNAL_COLUMNS) - set(signals.columns)
    if missing:
        raise RuntimeError(f"Frozen V5 signals missing columns: {sorted(missing)}")
    if len(signals) != int(manifest["cohort_structure"]["rows"]):
        raise RuntimeError("Frozen V5 signal row count differs from decision manifest")
...
def _load_fmp_key() -> str:
    try:
        ...
        raise RuntimeError(
            "Price snapshot builder requires the optional v5-builder dependencies"
        ) from exc
...
def fetch_ticker_prices(
    ...
    *,
    ...
    """Fetch canonical date/close rows from the same FMP v3 endpoint as production."""
...
def validate_price_snapshot(
    ...
    *,
    ...
    if stale:
        raise ValueError(
            "Price snapshot does not reach the required V5 market date for: "
            + ", ".join(stale)
        )
...
def require_outputs_absent() -> None:
    ...
    if existing:
        raise RuntimeError(
            "V5 price snapshot builder refuses to overwrite existing outputs: "
...
def main() -> None:
    ...
    try:
        ...
        raise RuntimeError(
            "Price snapshot builder requires requests; install the v5-builder extra"
        ) from exc
    ...
    summary = {
        "stage": "DTRM_PHASE3_MM1_V5_REALIZED_PRICE_SNAPSHOT",
        "status": "built_after_committed_exante_decision_before_realized_comparison",
        "decision_manifest": {
    ...
    print("DTRM PHASE 3 MM1 V5 REALIZED PRICE SNAPSHOT")
